...
import { Truck, TrendingUp, DollarSign, Plus, Eye, Receipt, Layers } from 'lucide-react';
import { WoodShipment, WoodShipmentStatus, WoodType, WoodGrade, WoodShipmentExpense, WoodExpenseStage } from '../types';

export function WoodShipments() {
  const { woodShipments, addWoodShipment, addExpenseToWoodShipment } = useApp();
  const [isNewShipmentDialogOpen, setIsNewShipmentDialogOpen] = useState(false);
  const [isAddExpenseDialogOpen, setIsAddExpenseDialogOpen] = useState(false);
  const [isDetailsDialogOpen, setIsDetailsDialogOpen] = useState(false);
  const [selectedShipment, setSelectedShipment] = useState<WoodShipment | null>(null);
  ...
  const [formData, setFormData] = useState({
    ...
    amountPaid: 0,
    status: 'commandé' as WoodShipmentStatus,
    woodType: 'Planche' as WoodType,
    panelThickness: '8mm' as '8mm' | '10mm' | '15mm',
    grade: 'Standard' as WoodGrade,
    dimensions: '',
    totalPieces: 0,
    unitPrice: 0,
  ...
  const [expenseForm, setExpenseForm] = useState({
    ...
    stage: 'Chargement (Cameroun)' as WoodExpenseStage,
    type: '',
    description: '',
    amount: '',
    referenceNumber: '',
  ...
  const generateShipmentId = () => {
    ...
    const count = woodShipments.length + 1;
    return `BOIS-${dateStr}-${String(count).padStart(3, '0')}`;
  };

  const handleSubmitShipment = () => {
    if (!formData.supplier || !formData.paymentReceiptNumber || !formData.bankPaymentDate ||
        formData.totalPieces === 0 || formData.unitPrice === 0 || formData.amountPaid === 0) {
      alert('Veuillez remplir tous les champs obligatoires');
      return;
    }

    const shipment: WoodShipment = {
      ...
      panelThickness: formData.woodType === 'Panneau' ? formData.panelThickness : undefined,
      grade: formData.grade,
      dimensions: formData.dimensions,
      totalPieces: formData.totalPieces,
      piecesSold: 0,
      unitPrice: formData.unitPrice,
      basePurchasePrice: formData.totalPieces * formData.unitPrice,
      expenses: [],
  ...
  const openAddExpenseDialog = (shipment: WoodShipment) => {
  ...
  const handleAddExpense = () => {
    ...
    const expense: WoodShipmentExpense = {
      id: Date.now().toString(),
    ...
    addExpenseToWoodShipment(selectedShipment.id, expense);
    resetExpenseForm();
  ...
  const getTotalExpenses = (shipment: WoodShipment) => {
    return shipment.expenses.reduce((sum, exp) => sum + exp.amount, 0);
  };

  const getTotalCost = (shipment: WoodShipment) => {
    return shipment.amountPaid + getTotalExpenses(shipment);
  };

  const getCostPerPiece = (shipment: WoodShipment) => {
    return shipment.totalPieces > 0 ? getTotalCost(shipment) / shipment.totalPieces : 0;
  };

  const getRemainingPieces = (shipment: WoodShipment) => {
    return shipment.totalPieces - shipment.piecesSold;
  };

  const getNetProfit = (shipment: WoodShipment) => {
    return shipment.totalRevenue - getTotalCost(shipment);
  };

  const getProfitMargin = (shipment: WoodShipment) => {
    return shipment.totalRevenue > 0 ? (getNetProfit(shipment) / shipment.totalRevenue) * 100 : 0;
  };

  const getStatusColor = (status: WoodShipmentStatus) => {
    const colors = {
  ...
  const getRowColor = (shipment: WoodShipment) => {
  ...
  const activeShipments = useMemo(() => {
    return woodShipments.filter(s => s.status === 'en stock');
  }, [woodShipments]);

  const totalActivePieces = useMemo(() => {
    return activeShipments.reduce((sum, s) => sum + getRemainingPieces(s), 0);
  }, [activeShipments]);

  const totalStockValue = useMemo(() => {
    return activeShipments.reduce((sum, s) => sum + (getRemainingPieces(s) * getCostPerPiece(s)), 0);
  }, [activeShipments]);
  ...
  const totalPotentialProfit = useMemo(() => {
    return activeShipments.reduce((sum, s) => {
      ...
      const remaining = getRemainingPieces(s);
      return sum + (remaining * (avgPrice - getCostPerPiece(s)));
    }, 0);
  }, [activeShipments]);

  const getExpenseTypesByStage = (stage: WoodExpenseStage): string[] => {
    const types = {
    ...
    <div className="space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600 dark:text-green-400">{totalActivePieces} pièces</p>
              </div>
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle className="flex items-center gap-2">
              ...
              Cargaisons de Bois (Plafond)
              <Badge variant="outline" className="ml-2">{woodShipments.length} cargaisons</Badge>
            </CardTitle>
              ...
              <table className="dark:text-gray-200 w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-700 border-b-2 border-gray-200 dark:border-gray-700">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300">Type de Bois</th>
                  ...
                  {woodShipments.map((shipment) => (
                    <tr key={shipment.id} className={`${getRowColor(shipment)} hover:opacity-75 transition-opacity`}>
                      ...
                      <td className="px-3 py-2">
                        <div>
                          <p className="font-medium text-gray-800 dark:text-gray-200">{shipment.woodType}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">{shipment.grade}</p>
                        </div>
                      </td>
                      <td className="px-3 py-2 text-right font-medium text-blue-700">{shipment.totalPieces}</td>
                      <td className="px-3 py-2 text-right font-medium text-orange-700">{shipment.piecesSold}</td>
                      <td className="px-3 py-2 text-right font-bold text-green-700">{getRemainingPieces(shipment)}</td>
      ...
      <Dialog open={isNewShipmentDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Nouvelle Cargaison de Bois</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setFormData({ ...formData, status: value as WoodShipmentStatus })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
            ...
            <Card className="bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800">
              <CardHeader>
                <CardTitle className="text-base">Section 2 - Détails du Bois</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="Lambourde">Lambourde</SelectItem>
                        <SelectItem value="Plafond">Plafond</SelectItem>
                        <SelectItem value="Panneau">Panneau</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  {formData.woodType === 'Panneau' && (
                    <div>
                      <Label htmlFor="panelThickness">Épaisseur du Panneau *</Label>
                      <Select
                        value={formData.panelThickness}
                        onValueChange={(value) => setFormData({ ...formData, panelThickness: value as '8mm' | '10mm' | '15mm' })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <div>
                    <Label htmlFor="grade">Qualité/Grade *</Label>
                    <Select
                      value={formData.grade}
                      onValueChange={(value) => setFormData({ ...formData, grade: value as WoodGrade })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Ex: 2.5m × 0.2m × 0.05m"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <Label htmlFor="totalPieces">Nombre Total de Pièces *</Label>
                    <Input
                      id="totalPieces"
                      type="number"
                  ...
                  <div>
                    ...
                    <div className="h-10 flex items-center px-3 bg-white border rounded-md">
                      <p className="font-bold text-green-600 dark:text-green-400">
                        {(formData.totalPieces * formData.unitPrice).toLocaleString()} FCFA
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    setExpenseForm({ ...expenseForm, stage: value as WoodExpenseStage, type: '' });
                  }}
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              {(['Chargement (Cameroun)', 'Déchargement (Tchad)'] as WoodExpenseStage[]).includes(expenseForm.stage) && (
                <div>
